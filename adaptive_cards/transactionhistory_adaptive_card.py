...
def transaction_history_card(data):
    ...
    transactionHistoryCard.add(
        TextBlock(text=str(data['account']['accountType'])+"  "+str(data['account']['accountNo'])+" - Balance is  "+str(data['account']['currencySymbol'])+str(data['account']['balance']), color="light", size="Medium",
                  weight="Bolder"))
    transactionHistoryCard.add(
